...
def test_two_connections_racing_apply_pending_do_not_double_apply(
    ...
    async def _run() -> list[list]:
        ...
        try:
            await conn_hold.execute("DROP TABLE IF EXISTS schema_migrations")
            await conn_hold.execute("DROP TABLE IF EXISTS lock_test_t")
            ...
            await conn_hold.execute(
                "DROP TABLE IF EXISTS lock_test_t"
            )
