...
def test_actionable_findings_use_request_changes_workflow() -> None:
    """This is the mechanism that blocks merge: CodeRabbit submits a GitHub
    "Request changes" review instead of a plain comment for any actionable
    finding. There is no severity threshold -- this is an all-or-nothing
    switch, not a filter.
    """
    data = _load()
...
def test_path_instructions_restate_key_business_rules() -> None:
    data = _load()
    combined = " ".join(
        entry["instructions"] for entry in data["reviews"]["path_instructions"]
    )
