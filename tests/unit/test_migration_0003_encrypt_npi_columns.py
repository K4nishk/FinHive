...
def test_migration_0003_replaces_npi_with_ct() -> None:
    ...
    for body in (loans_body, loan_history_body):
        for column in _LOANS_AND_LOAN_HISTORY_NPI_COLUMNS:
            ...
            assert f"{drop}," in body or f"{drop}\n" in body, (
...
def test_migration_0003_report_records_encrypts_npi() -> None:
    sql = _migration_0003_sql()
...
def test_migration_0003_leaves_rates_periods_and_dates_plaintext() -> None:
    ...
    for column in _REPORT_RECORDS_PLAINTEXT_COLUMNS:
        assert f"DROP COLUMN {column}" not in body, (
            f"{column} must stay plaintext"
        )
        assert f"{column}_ct" not in sql, (
            f"{column} must not gain a _ct column"
        )
