...
def test_migration_0004_adds_a_bidx_column_for_every_identity_column() -> None:
    ...
    for body in (loans_body, loan_history_body):
        for column in _LOANS_AND_LOAN_HISTORY_IDENTITY_COLUMNS:
            assert (
                f"ADD COLUMN {column}_bidx BYTEA"
                in body
            ), f"missing {column}_bidx"


def test_migration_0004_rr_adds_bidx_for_identity() -> None:
    sql = _migration_0004_sql()
...
def test_migration_0004_indexes_every_bidx_column_it_adds() -> None:
    ...
    for table, columns in (
        ...
        for column in columns:
            idx = (
                f"CREATE INDEX"
                f" {table}_{column}_bidx_idx"
                f" ON {table}"
                f" ({column}_bidx)"
            )
            assert idx in sql, (
                f"{table}.{column}_bidx"
                " is not indexed"
            )


def test_migration_0004_no_bidx_for_amount() -> None:
    ...
    for column in FORBIDDEN_BLIND_INDEX_COLUMNS:
        assert f"{column}_bidx" not in sql, (
            f"{column} must never gain a blind index"
        )
...
def test_migration_0004_does_not_touch_amount_bearing_ct_columns() -> None:
    ...
    for column in FORBIDDEN_BLIND_INDEX_COLUMNS:
        assert f"{column}_ct" not in sql, (
            f"{column}_ct must not be touched"
            " by this migration"
        )
