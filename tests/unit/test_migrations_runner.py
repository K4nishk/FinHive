...
Acceptance: running the migrator twice is idempotent,
and editing an already applied migration fails.
``apply_pending`` is exercised directly (not just the
pure ``plan_migrations`` helper it's built on) against
a fake connection, so this covers the actual code path
the CLI runs -- not just the planning logic underneath
it. No Postgres or asyncpg required: the fake
connection duck-types the ``execute``/``fetch``/
``transaction`` surface ``apply_pending`` depends on.
"""
...
def test_discover_migrations_rejects_short_version_prefix(
    tmp_path: Path,
) -> None:
...
def test_plan_is_up_to_date_when_nothing_pending_or_mismatched(
    tmp_path: Path,
) -> None:
    _write(tmp_path, "0001_init.sql", "CREATE TABLE t (a INT);")
    migrations = discover_migrations(tmp_path)
...
def test_plan_flags_edited_applied_migration_as_mismatched(
    tmp_path: Path,
) -> None:
    _write(tmp_path, "0001_init.sql", "CREATE TABLE t (a INT);")
    migrations = discover_migrations(tmp_path)
...
def test_plan_flags_renamed_applied_migration_as_renamed(
    tmp_path: Path,
) -> None:
...
# -- apply_pending (async, fake connection) -----------------
...
def test_apply_pending_releases_the_advisory_lock_on_failure(
    tmp_path: Path,
) -> None:
...
def test_apply_pending_raises_before_applying_pending_on_mismatch(
    tmp_path: Path,
) -> None:
...
def test_apply_pending_fails_when_new_file_below_highest_applied(
    tmp_path: Path,
) -> None:
...
def test_cli_returns_one_on_migration_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    async def _raise_checksum_mismatch(
        conn: object, migrations_dir: Path
    ) -> list:
        raise ChecksumMismatchError(
            1, "init", "expected-checksum", "actual-checksum"
        )

    monkeypatch.setattr(
        "finhive.db.migrations.apply_pending", _raise_checksum_mismatch
    )
