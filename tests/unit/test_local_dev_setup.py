...
RUN_FULL_STACK = os.environ.get("FINHIVE_FULL_STACK_INTEGRATION") == "1"
...
def _api_responds(host: str, port: int) -> bool:
    """True if the API serves an HTTP response on /api/me."""
    try:
...
def _launch_and_verify_spa(
    script_args: list[str],
    spa_port: int = 5173,
    api_port: int | None = None,
    timeout: int = 300,
) -> None:
    if _port_is_open("127.0.0.1", spa_port):
        pytest.fail(
            f"port {spa_port} is already occupied before launch"
        )
    ...
    try:
        ...
        while time.monotonic() < deadline:
            if _spa_responds("127.0.0.1", spa_port):
                spa_up = True
                break
            ...
            time.sleep(1)
        assert spa_up, (
            f"SPA did not respond on :{spa_port} within {timeout}s"
        )
        if api_port is not None:
            assert _api_responds("127.0.0.1", api_port), (
                f"API did not serve HTTP on :{api_port}/api/me"
            )
    finally:
...
@pytest.mark.skipif(
    not RUN_INTEGRATION,
    reason=(
        "opt-in: set FINHIVE_LOCAL_SETUP_INTEGRATION=1 to actually "
        "launch run_local_mac.sh and verify the SPA serves real "
        "traffic. Requires Python 3.10+, Node 20+, and either "
        "DATABASE_URL or a running Supabase CLI plus a container "
        "runtime."
    ),
)
...
@pytest.mark.skipif(
    not RUN_FULL_STACK,
    reason=(
        "opt-in: set FINHIVE_FULL_STACK_INTEGRATION=1 to run "
        "run_local_mac.sh WITHOUT --allow-partial. Requires "
        "KCH-91, KCH-94, and KCH-102 to have landed."
    ),
)
