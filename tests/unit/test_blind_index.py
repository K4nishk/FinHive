...
# `compute_blind_index` takes an already-derived `key_index`, not a master
# or `key_data` -- derive it once here the way a real caller (KeyRing) would.
...
def test_derive_key_index_is_deterministic_for_the_same_master() -> None:
...
def test_blind_index_is_16_bytes() -> None:
    index = compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_name")
...
def test_same_plaintext_and_key_always_produce_the_same_index() -> None:
    ...
    """
    first = compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_name")
    second = compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_name")
...
def test_index_is_stable_regardless_of_case_or_padding() -> None:
    ...
    assert (
        compute_blind_index("  SHARMA TRADERS  ", _KEY_INDEX, column="borrower_name")
        == canonical
    )
    assert (
        compute_blind_index("sharma traders", _KEY_INDEX, column="borrower_name")
...
def test_distinct_names_produce_distinct_indexes() -> None:
    ...
    """
    sharma = compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_name")
    gupta = compute_blind_index("Gupta Finance", _KEY_INDEX, column="borrower_name")
...
def test_name_and_group_lookups_agree_for_group_auto_fill() -> None:
    ...
    """
    name_index = compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_name")
    group_index = compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_group")
    ...
    assert name_index == compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_name")
    assert group_index == compute_blind_index("Sharma Traders", _KEY_INDEX, column="borrower_group")
...
def test_different_keys_produce_different_indexes_for_the_same_plaintext() -> None:
    ...
    under_other_key = compute_blind_index(
        "Sharma Traders", _OTHER_KEY_INDEX, column="borrower_name"
    )
...
def test_every_identity_column_is_accepted(column: str) -> None:
    index = compute_blind_index("Sharma Traders", _KEY_INDEX, column=column)
...
def test_compute_blind_index_refuses_every_amount_column(column: str) -> None:
    ...
    with pytest.raises(ValueError, match="ADR-2.4"):
        compute_blind_index("150000.00", _KEY_INDEX, column=column)


def test_compute_blind_index_refuses_an_unknown_column() -> None:
    with pytest.raises(ValueError):
        compute_blind_index("whatever", _KEY_INDEX, column="reference_id")
