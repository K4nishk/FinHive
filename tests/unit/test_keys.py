...
def test_derive_key_data_and_derive_key_index_never_collide() -> None:
    ...
    key_index = derive_key_index(_MASTER)

    assert key_data != key_index


@pytest.mark.parametrize("bad_key", [b"\x00" * 16, b"\x00" * 31, b"\x00" * 33])
def test_derive_key_data_rejects_a_master_that_is_not_32_bytes(
    bad_key: bytes,
) -> None:
...
def test_load_key_ring_reads_the_current_version_and_its_master() -> None:
    ring = load_key_ring(
        {"FINHIVE_KEY_VERSION": "1", "FINHIVE_MASTER_KEY_V1": _b64(_MASTER)}
    )
...
def test_load_key_ring_rejects_a_master_of_the_wrong_length() -> None:
    with pytest.raises(ConfigError, match="32 bytes"):
        load_key_ring(
            {
                "FINHIVE_KEY_VERSION": "1",
                "FINHIVE_MASTER_KEY_V1": base64.b64encode(
                    b"\x00" * 16
                ).decode(),
            }
        )
...
def test_key_ring_key_index_matches_derive_key_index_of_the_master() -> None:
    ring = KeyRing(current_version=1, masters={1: _MASTER})

    assert ring.key_index() == derive_key_index(_MASTER)


def test_key_ring_key_index_does_not_chain_through_key_data() -> None:
    """Security regression: `key_index` must be derived directly from the
    master, never from `key_data` -- otherwise exposing `key_data` (which
    happens on every encrypt/decrypt call) would also hand over the
    blind-index key. See ADR-2.3 "Key management".
    """
    ring = KeyRing(current_version=1, masters={1: _MASTER})
...
def test_rotate_field_re_encrypts_under_the_new_version(
    two_version_ring: KeyRing,
) -> None:
    old_blob = encrypt_field("Sharma Traders", two_version_ring.key_data(1))

    new_blob = rotate_field(
        old_blob, from_version=1, to_version=2, ring=two_version_ring
    )

    assert (
        decrypt_field(new_blob, two_version_ring.key_data(2))
        == "Sharma Traders"
    )


def test_rotated_blob_no_longer_decrypts_under_the_old_version(
    two_version_ring: KeyRing,
) -> None:
    old_blob = encrypt_field("Sharma Traders", two_version_ring.key_data(1))
    new_blob = rotate_field(
...
def test_rotate_identity_field_recomputes_a_matching_blind_index(
    two_version_ring: KeyRing,
) -> None:
    old_blob = encrypt_field("Sharma Traders", two_version_ring.key_data(1))
...
def test_rotating_a_subset_leaves_the_ring_able_to_read_both_versions(
    ...
    rows = {
        "sharma": (
            encrypt_field("Sharma Traders", two_version_ring.key_data(1)),
            1,
        ),
        "gupta": (
            encrypt_field("Gupta Finance", two_version_ring.key_data(1)),
            1,
        ),
        "verma": (
            encrypt_field("Verma Loans", two_version_ring.key_data(1)),
            1,
        ),
    }
