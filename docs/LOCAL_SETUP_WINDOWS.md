...
`SUPABASE_URL` defaults to `supabase start`'s local address if unset.
...
`run_local_windows.bat`) is idempotent: it looks the org and auth user up by
...
FINHIVE_MASTER_KEY_V1=<base64 output from above>
```
...
Once those land, re-running `run_local_windows.bat` (no flag needed) picks
