...
Confines the ``service_role`` key to this one module
(ARD v2.0.0 section 8, Gotcha 3):
...
_PER_PAGE = 200
_MAX_PAGES = 5  # local/dev only -- bounds the scan
_TIMEOUT_S = 30
...
class AdminClient:
    ...
    def __init__(
        ...
        if opener is urlrequest.urlopen:
            opener = partial(
                urlrequest.urlopen,
                timeout=_TIMEOUT_S,
            )
    ...
    def create_or_get_user(self, email: str, password: str) -> str:
        """Return the auth user id for *email*."""
    ...
    def _try_create_user(self, email: str, password: str) -> str | None:
        ...
        if not user_id:
            raise SupabaseAdminError(
                "create user response had no id:"
                f" {payload!r}"
            )
        return str(user_id)
