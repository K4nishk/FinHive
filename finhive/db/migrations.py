...
Reads numbered ``migrations/NNNN_name.sql`` files and
applies the ones a target Postgres database hasn't seen
yet, recording each in a ``schema_migrations`` table
keyed by version, checksum and applied-at timestamp.
There are no down-migrations: rolling a live schema back
is riskier than rolling forward, so the only way to undo
a change is a new migration that does so.

The planning logic (``discover_migrations``,
``plan_migrations``) is pure and has no database
dependency, so it's fully unit-testable without Postgres
or asyncpg installed. ``apply_pending`` is the only
function that talks to a connection, and it duck-types
against one (``execute`` / ``fetch`` / ``transaction``)
rather than importing asyncpg, so callers can pass a
real ``asyncpg.Connection`` or a test double.
"""
...
class ChecksumMismatchError(MigrationError):
    ...
    def __init__(
        self, version: int, name: str, expected: str, actual: str
    ) -> None:
        self.version = version
        self.name = name
        ...
        self.actual = actual
        super().__init__(
            f"migration {version:04d}_{name}.sql was"
            f" already applied with checksum"
            f" {expected}, but the file on disk now"
            f" checksums to {actual}. Migrations"
            " are forward-only and immutable once"
...
class NameMismatchError(MigrationError):
    ...
    def __init__(self, version: int, expected: str, actual: str) -> None:
        ...
        self.actual = actual
        super().__init__(
            f"migration {version:04d} was recorded"
            f" as applied under the name"
            f" '{expected}', but the file on disk"
            f" for that version is now named"
            f" '{actual}'. Migrations are"
            " forward-only and immutable once"
            " applied -- renaming an applied"
            " migration's file is not allowed."
        )
...
class MissingMigrationError(MigrationError):
    ...
    def __init__(self, version: int) -> None:
        self.version = version
        super().__init__(
            f"migration {version:04d} is recorded as"
            " applied in schema_migrations, but"
            " no matching file exists in the"
            " migrations directory. Migrations"
            " are forward-only and immutable once"
            " applied -- restore the file instead"
            " of deleting it."
        )
...
def discover_migrations(migrations_dir: Path) -> list[MigrationFile]:
    ...
    for path in migrations_dir.iterdir():
        # Case-insensitive so 0001_init.SQL reaches
        # the name check below and raises, instead
        # of silently missing on a case-sensitive fs.
...
def plan_migrations(
    ...
    if highest_applied is not None:
        out_of_order = sorted(
            (m for m in pending if m.version < highest_applied),
...
async def apply_pending(
    ...
    """
    rows = await conn.fetch("SHOW lock_timeout")
    saved_lock_timeout = rows[0]["lock_timeout"]
    lock_acquired = False
    try:
        await conn.execute(
            f"SET lock_timeout = '{_LOCK_TIMEOUT_SECONDS}s'"
        )
        try:
            await conn.execute(
            ...
            raise MigrationError(
                "failed to acquire the migration"
                " advisory lock"
            ) from exc
        lock_acquired = True
        await conn.execute(
        ...
        if not result.is_clean:
            ...
            if result.renamed:
                ...
                raise NameMismatchError(
                    migration.version, record.name, migration.name
                )
...
async def _run_cli() -> int:
    parser = argparse.ArgumentParser(
        description="Forward-only SQL migration runner"
    )
    parser.add_argument(
