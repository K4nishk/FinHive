...
import { authReducer, initialAuthState } from "./authReducer.ts";
import type { MeResponse, Session } from "./types.ts";
...
test("initial state is loading with no session or me", () => {
  assert.deepEqual(initialAuthState, { session: null, me: null, status: "loading" });
});

test("SESSION_CHANGED with a session keeps status until me loads", () => {
  const next = authReducer(initialAuthState, { type: "SESSION_CHANGED", session });
...
test("SESSION_CHANGED with null signs the user out", () => {
  ...
  const next = authReducer(authenticated, { type: "SESSION_CHANGED", session: null });
  assert.deepEqual(next, { session: null, me: null, status: "unauthenticated" });
});
...
test("REFRESHED replaces session and me in a single transition", () => {
  ...
  const withSession = { session, me: null, status: "loading" as const };
  const next = authReducer(withSession, { type: "REFRESHED", session: refreshedSession, me });
  assert.deepEqual(next, { session: refreshedSession, me, status: "authenticated" });
});
