...
import { isRoleAllowed, resolveRoleGateDecision } from "./roleGateLogic.ts";
...
test("isRoleAllowed: an unresolved role (undefined) is never allowed", () => {
  assert.equal(isRoleAllowed(undefined, ["owner", "bookkeeper", "viewer"]), false);
});

test("resolveRoleGateDecision: allowed role renders children regardless of variant", () => {
  assert.deepEqual(resolveRoleGateDecision("owner", ["owner"], "hide", "/"), { kind: "children" });
  assert.deepEqual(resolveRoleGateDecision("owner", ["owner"], "disable", "/"), { kind: "children" });
  assert.deepEqual(resolveRoleGateDecision("owner", ["owner"], "redirect", "/"), { kind: "children" });
});

test("resolveRoleGateDecision: disallowed + hide falls back", () => {
  assert.deepEqual(resolveRoleGateDecision("viewer", ["owner"], "hide", "/"), { kind: "fallback" });
});

test("resolveRoleGateDecision: disallowed + disable disables", () => {
  assert.deepEqual(resolveRoleGateDecision("viewer", ["owner"], "disable", "/"), { kind: "disable" });
});

test("resolveRoleGateDecision: disallowed + redirect carries the redirect target", () => {
  assert.deepEqual(resolveRoleGateDecision("viewer", ["owner"], "redirect", "/dashboard"), {
    kind: "redirect",
    to: "/dashboard",
  });
});
